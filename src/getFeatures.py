...
def DRLF_Embed(fastaFile,outFile,device=-2):
    ...
    print("SSA Embedding...") 
    lm_embed, lstm_stack, proj=load_model("./src/PretrainedModel/SSA_embed.model", use_cuda=False)
    ...
    with open(path, 'rb') as f:
        for name,sequence in fasta.parse_stream(f):
            ...
            z = embed_sequence(sequence, lm_embed, lstm_stack, proj
                               ...
                               pool='avg'
                                  , use_cuda=False)
            SSAEMB_.append(z.astype(np.float32))
            count += 1
    ...
    #         UNIREPEB_.append(unirep_output.astype(np.float32))
    #         count += 1
    #         print(sequence,'# {} sequences processed...'.format(count), file=sys.stderr, end='\r')

    #Features = []
    #for i in range(len(SSAEMB_)):
    #    Features.append(torch.cat((torch.from_numpy(SSAEMB_[i]), torch.zeros(50 - SSAEMB_[i].shape[0], 121, dtype=torch.float)), dim=0))

    return SSAEMB_,inData
